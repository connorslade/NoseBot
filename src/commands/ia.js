        ...
        let Loading = await msg.channel.send(common.embedMessage(color.link, `Loading Instant Answer...`, 'Hang Tight!'));
            ...
            response.on('end', () => {
                ...
                if (jsonResponse['Abstract'] === "" && jsonResponse['RelatedTopics'][0] === undefined) {
                    Loading.edit(common.embedMessage(color.red, `**Search:** ${jsonResponse['Heading']} {BETA}`, `No Instant Answer for ${working}`));
                    return;
                }
                ...
                if (!(jsonResponse['Abstract'] === "")) {
                    ...
                    Loading.edit(common.embedMessage(color.nose, `**Search:** ${jsonResponse['Heading']} {BETA}`, bodyContent).setURL(jsonResponse['AbstractURL']).setFooter(footerContent));
                    return;
                }
